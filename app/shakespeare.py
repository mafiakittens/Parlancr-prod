...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.online_testing:
            while True:
                #The next 5 lines create a prompt and ingest the input typed into that prompt, e.g.
                ...
                if inp == 'quit' or inp == 'exit':
                    break
                #Split the input into component words
                inp = inp.split()
                #Assign the first character, "1" above, to y (the valid values are "1" or "0" to
                #signify the style of the input text
                y = int(inp[0])
                #Assign the remaining characters, the rest of the words in the string, to sent
                sent = inp[1:]

                #Send the code and the string to get_batch, it will be converted to an embedding found
                #in vocab.word2id and the output is assigned to batch
                batch = get_batch([sent], [y], vocab.word2id)
                #Get decoder to translate the embedded sentence from batch. There are two outputs
                #Output 1 (ori) is the original input translated into the orignal source style
                #Output 2 (tsf) is the original input translated into the new target style
                ori, tsf = decoder.rewrite(batch)
                #The next two lines just print the results to the screen.
